...
module.exports = {
  async byUsername(username, ctx, next) {
    ...
    ctx.params.profile = await db('users')
      .select(...getSelect('users', 'profile', userFields), 'followers.id as profile_following')
      .where({ username })
      .leftJoin('followers', function() {
        this.on('users.id', '=', 'followers.user').onIn('followers.follower', [user && user.id]);
      });

    ctx.assert(ctx.params.profile && ctx.params.profile.length, 404);

    ctx.params.profile = joinJs.mapOne(ctx.params.profile, relationsMaps, 'userMap', 'profile_');
