...
module.exports = {
  ...
  async get(ctx) {
    ...
    articlesQuery = articlesQuery
      ...
      .leftJoin('favorites', function() {
        this.on('articles.id', '=', 'favorites.article').onIn('favorites.user', [user && user.id]);
      })
      .leftJoin('followers', function() {
        this.on('articles.author', '=', 'followers.user').onIn('followers.follower', [user && user.id]);
      });

    let [articles, [countRes]] = await Promise.all([articlesQuery, countQuery]);

    articles = joinJs.map(articles, relationsMaps, 'articleMap', 'article_').map((a) => {
  ...
  async post(ctx) {
    ...
    if (article.tagList && article.tagList.length > 0) {
      tags = await Promise.all(
        article.tagList.map((t) => ({ id: uuid(), name: t })).map((t) => ctx.app.schemas.tag.validate(t, opts))
      );
    }

    try {
      await db('articles').insert(humps.decamelizeKeys(_.omit(article, ['tagList'])));
    } catch (err) {
      ctx.assert(parseInt(err.errno, 10) === 19 || parseInt(err.code, 10) === 23505, err);
  ...
  async put(ctx) {
    ...
    ctx.assert(article.author.id === ctx.state.user.id, 422, new ValidationError(['not owned by user'], '', 'article'));
    ...
    newArticle = await ctx.app.schemas.article.validate(humps.camelizeKeys(newArticle), opts);
    ...
    if (fields.tagList && fields.tagList.length > 0) {
      if (_.difference(article.tagList).length || _.difference(fields.tagList).length) {
        ...
        let tags = await Promise.all(
          newArticle.tagList.map((t) => ({ id: uuid(), name: t })).map((t) => ctx.app.schemas.tag.validate(t, opts))
        );
