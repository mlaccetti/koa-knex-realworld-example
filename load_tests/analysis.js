...
const analysis = {
  getErrorBudget: (script, defaultErrorBudget) => {
    if (
      script &&
      script.sampling &&
      script.sampling.errorBudget !== undefined
    ) {
  ...
  analyzeSamples: (script, report, defaultErrorBudget) => {
    ...
    const totalErrors = Object.keys(totals.errors).reduce(
      (total, message) => total + totals.errors[message],
      0,
    )
    ...
    if (totalErrors > errorBudget) {
      finalReport.errorMessage = `${modes.getModeForDisplay(
        script,
      )} failure: scenarios run: ${
        totals.scenariosCreated
      }, total errors: ${totalErrors}, error budget: ${errorBudget}`
    }
  ...
  analyzeAcceptance: (timeNow, script, settings, results) =>
    analysis.analyzeSamples(
      script,
      results,
      sampling.defaults.acceptance.errorBudget,
    ),
  ...
  analyzeMonitoring: (timeNow, script, settings, results) =>
    analysis.analyzeSamples(
      script,
      results,
      sampling.defaults.monitoring.errorBudget,
    ),
