const sampling = {
  ...
  validateSampling: (script, settings) => {
    ...
    if (typeof script.sampling.size !== "number" || script.sampling.size <= 0) {
      throw new Error(
        "If specified, the sampling size must have type number and be greater than zero",
      )
    }

    if (
      typeof script.sampling.averagePause !== "number" ||
      script.sampling.averagePause <= 0
    ) {
      throw new Error(
        "If specified, the sampling averagePause must have type number and be greater than zero",
      )
    }

    if (
      typeof script.sampling.pauseVariance !== "number" ||
      script.sampling.pauseVariance < 0
    ) {
      throw new Error(
        "If specified, the sampling pauseVariance must have type number and be greater than or equal to zero",
      )
    }

    if (
      typeof script.sampling.errorBudget !== "number" ||
      script.sampling.errorBudget < 0
    ) {
      throw new Error(
        "If specified, the sampling errorBudget must have type number and be greater than or equal to zero",
      )
    }

    if (
      typeof script.sampling.warningThreshold !== "number" ||
      script.sampling.warningThreshold <= 0 ||
      script.sampling.warningThreshold > 1
    ) {
      throw new Error(
        "If specified, the sampling warningThreshold must have type number and be either one or between zero and one",
      )
    }
    ...
    if (totalSamples <= script.sampling.errorBudget) {
      throw new Error(
        `The given size * scenarios (${totalSamples}) and errorBudget (${
          script.sampling.errorBudget
        }) values (perhaps from defaults) succeeds even if all samples fail`,
      )
    }

    if (script.sampling.pauseVariance > script.sampling.averagePause) {
      throw new Error(
        `The given pauseVariance (${
    ...
    const mostPossiblePause =
      (script.sampling.averagePause + script.sampling.pauseVariance) *
      totalSamples

    if (mostPossiblePause > settings.maxScriptDurationInSeconds) {
      throw new Error(
        `The given averagePause (${
          script.sampling.averagePause
        }), pauseVariance (${
          script.sampling.pauseVariance
        }), and size * scenarios (${totalSamples}) values in combination could, even ignoring request duration, exceed the maximum allowable duration (${
    ...
    if (
      ...
      console.warn(
        [
          ...
          `\t(${script.sampling.averagePause} + ${
            script.sampling.pauseVariance
          }) * ${script.sampling.size} = ${mostPossiblePause}`,
          `\t${settings.maxScriptDurationInSeconds} * ${
            script.sampling.warningThreshold
          } = ${settings.maxScriptDurationInSeconds *
            script.sampling.warningThreshold}`,
          "Condition",
          `\t${mostPossiblePause} > ${settings.maxScriptDurationInSeconds *
            script.sampling.warningThreshold}`,
        ].join("\n"),
      )
    }
