...
const planning = {
  ...
  scriptDurationInSeconds: script => {
    ...
    for (i = 0; i < script.config.phases.length; i++) {
      phaseDurationInSeconds = planning.phaseDurationInSeconds(
  ...
  splitScriptByDurationInSeconds: (script, chunkSize) => {
    ...
    while (
      ...
      if (phaseDurationInSeconds > remainingDurationInSeconds) {
        // split phase
        phaseParts = planning.splitPhaseByDurationInSeconds(
          phase,
  ...
  splitScriptByRequestsPerSecond: (script, chunkSize) => {
    ...
    for (i = 0; i < script.config.phases.length; i++) {
      phaseParts = planning.splitPhaseByRequestsPerSecond(
        script.config.phases[i],
        chunkSize,
      )
  ...
  splitScriptByDurationInSecondsAndSchedule: (timeNow, event, settings) => {
    ...
    const parts = planning.splitScriptByDurationInSeconds(
      script,
    ...
    if (script._trace) {
      console.log(
        `scheduling immediate chunk start from ${
          script._genesis
    ...
    if (script._trace) {
      console.log(
        `scheduling future chunk start from ${
          script._genesis
  ...
  splitScriptByRequestsPerSecondAndSchedule: (timeNow, event, settings) => {
    ...
    if (script._trace) {
      console.log(
        `splitting immediate chunk by requests per second from ${
          script._genesis
        } in ${timeNow} @ ${Date.now()}`,
      )
    }
    ...
    do {
      const parts = planning.splitScriptByRequestsPerSecond(
        script,
    ...
    if (script._trace) {
      console.log(
        `immediate chunk split in to ${plan.length -
          initialLength} chunks, with ${initialLength} future chunk(s) from ${
          script._genesis
        } in ${timeNow} @ ${Date.now()}`,
      )
    }
  ...
  splitScriptByFlow: script => {
    ...
    for (i = 0; i < oldScript.scenarios.length; i++) {
      ...
      if (oldScript.scenarios[i].flow) {
        ...
        newScript.config.phases = planning.generateSamplingPhases(
          script.sampling,
        ) // do this for every script so that they don't act in sync and create harmonic effects
  ...
  planPerformance: (timeNow, script, settings) => {
    ...
    // ## Duration ##
    const scriptDurationInSeconds = planning.scriptDurationInSeconds(
      updatedScript,
    )
    ...
    if (scriptDurationInSeconds > settings.maxChunkDurationInSeconds) {
      const parts = planning.splitScriptByDurationInSecondsAndSchedule(
        timeNow,
        updatedScript,
        settings,
      )
    ...
    // ## Requests Per Second ##
    const scriptRequestsPerSecond = planning.scriptRequestsPerSecond(
      updatedScript,
    )
    ...
    if (scriptRequestsPerSecond <= settings.maxChunkRequestsPerSecond) {
      ...
      const parts = planning.splitScriptByRequestsPerSecondAndSchedule(
        timeNow,
        updatedScript,
        settings,
      )
