...
const platformSettings = {
  ...
  validate: (script, settings) => {
    ...
    if (script && script._split && typeof script._split !== "object") {
      // eslint-disable-line no-underscore-dangle
      throw new Error(
        'If specified, the "_split" attribute must contain an object',
      )
    }
    ...
    const checkBoundedIntegerSetting = (setting, minimum, maximum) => {
      ...
      if (value) {
        ...
        if (isInvalidValue) {
          throw new Error(
            `If specified, the "_split.${setting}" attribute must be an integer ` +
    ...
    checkBoundedIntegerSetting(
      "maxChunkDurationInSeconds",
      platformSettings.MIN_CHUNK_DURATION_IN_SECONDS,
      platformSettings.MAX_CHUNK_DURATION_IN_SECONDS,
    )
    checkBoundedIntegerSetting(
      "maxScriptDurationInSeconds",
      1,
      platformSettings.MAX_SCRIPT_DURATION_IN_SECONDS,
    )
    checkBoundedIntegerSetting(
      "maxChunkRequestsPerSecond",
      1,
      platformSettings.MAX_CHUNK_REQUESTS_PER_SECOND,
    )
    checkBoundedIntegerSetting(
      "maxScriptRequestsPerSecond",
      1,
      platformSettings.MAX_SCRIPT_REQUESTS_PER_SECOND,
    )
    checkBoundedIntegerSetting(
      "timeBufferInMilliseconds",
      1,
      platformSettings.MAX_TIME_BUFFER_IN_MILLISECONDS,
    )
  },
  ...
  getSettings: script => {
    ...
    const settings = Object.assign(
      {
        maxScriptDurationInSeconds:
          platformSettings.DEFAULT_MAX_SCRIPT_DURATION_IN_SECONDS,
        maxScriptRequestsPerSecond:
          platformSettings.DEFAULT_MAX_SCRIPT_REQUESTS_PER_SECOND,
        maxChunkDurationInSeconds:
          platformSettings.DEFAULT_MAX_CHUNK_DURATION_IN_SECONDS,
        maxChunkRequestsPerSecond:
          platformSettings.DEFAULT_MAX_CHUNK_REQUESTS_PER_SECOND,
        timeBufferInMilliseconds:
          platformSettings.DEFAULT_MAX_TIME_BUFFER_IN_MILLISECONDS,
      },
      split,
    )
