...
const artilleryTask = {
  ...
  invokeSelf(timeDelay, script, invocationType) {
    ...
    trace(
      `scheduling self invocation for ${script._genesis} in ${
        script._start
  ...
  distribute: (timeNow, script, settings, plans) => {
    ...
    trace(
      `distributing ${plans.length} plans from ${
        script._genesis
      } in ${timeNow}`,
    )
  ...
  execute: (timeNow, script) =>
    new Promise((resolve, reject) => {
      ...
      const msg = `ERROR exception encountered while executing load from ${
        script._genesis
      } in ${timeNow}: ${ex.message}\n${ex.stack}`
      console.error(msg)
      throw new Error(msg)
    }),
